...
function changeFields(id) {
  ...
  addBtn.classList =
    "btn  add-field align-self-start position-absolute p-3 ml-2";
  ...
  spans.forEach(item => {
    ...
    <input type="text" class="form-control" placeholder="text" aria-label="Text input with checkbox" name="todo-text" value="${
      item.innerText
    }">
    <div class="input-group-append">
        <button class="btn btn-outline-secondary field-delete" type="button"></button>
...
function replaceFieldsNote(id) {
  ...
  editNoteForm.innerHTML = `<div class="form-group">
    <button style="top:5px; right:5px;" type="button" class="close note-delete-btn field-delete d-block p-2 position-absolute" aria-label="Close" data-id=${id}></button>
            ...
            <input name="note-title" type="text" class="form-control" id="InputTitle" placeholder="Enter title" value="${
              title.innerText
            }">
        </div>
        <div class="form-group">
            <label for="FormControlTextarea">text</label>
            <textarea name="note-text" class="form-control" id="FormControlTextarea" rows="12">${
              text.innerText
            }</textarea>
        </div>
